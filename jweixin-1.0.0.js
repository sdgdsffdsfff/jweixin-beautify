!function(a, b) {
    ...
    function e(config) {
        config = config || {};
        config.appId = z.appId;
        config.verifyAppId = z.appId;
        config.verifySignType = "sha1";
        config.verifyTimestamp = z.timestamp + "";
        config.verifyNonceStr = z.nonceStr;
        config.verifySignature = z.signature;
        return config;
    }
    function f(config) {
        return {
            ...
            signType: config.signType || "SHA1"
        }
    }
    function g(a, b, callback) {//c---callback
        var d, e, f;
        ...
        case "ok":
            callback.success && callback.success(b);
            break;
        case "cancel":
            callback.cancel && callback.cancel(b);
            break;
        default:
            callback.fail && callback.fail(b)
        }
        callback.complete && callback.complete(b)
    }
    function h(a, b) {
        var d, e, f, g;
        if (b) {
